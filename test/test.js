...
describe("RFergNFTContract", async function () {
  let RFergNFTContractFactory;
  let RFergNFTContract;
  let args = {
  ...
  this.beforeEach(async function () {
    RFergNFTContractFactory = await ethers.getContractFactory(
      "RFergNFTContract"
    );
    RFergNFTContract = await RFergNFTContractFactory.deploy(
      args.mint_price,
      args.max_tokens,
      args.base_uri,
      args.royaltyArtist,
    ...
    await RFergNFTContract.waitForDeployment(
      args.mint_price,
      args.max_tokens,
      args.base_uri,
      args.royaltyArtist,
  ...
  describe("construction and initialization", async function () {
    ...
    it("should be named RFergNFTContract", async function () {
      const expectedValue = "RFergNFTContract";
      const currentValue = await RFergNFTContract.name();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should be have symbol RFNFT", async function () {
      const expectedValue = "RFNFT";
      const currentValue = await RFergNFTContract.symbol();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should have a mint price set when constructed", async function () {
      const expectedValue = args.mint_price;
      const currentValue = await RFergNFTContract.getMintPrice();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should have a max token supply set when constructed", async function () {
      const expectedValue = args.max_tokens;
      const currentValue = await RFergNFTContract.getMaxSupply();
      assert.equal(currentValue.toString(), expectedValue);
    });

    it("should have a base URI set when constructed", async function () {
      const expectedValue = args.base_uri;
      const currentValue = await RFergNFTContract.getBaseURI();
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should set roylaty artist when constructed", async function () {
      ...
      const currentValue = await RFergNFTContract.royaltyInfo(
        1,
    ...
    it("should set owner to the deployer's address when constucted", async function () {
      ...
      const currentValue = await RFergNFTContract.owner();
      assert.equal(currentValue.toString(), expectedValue);
    });
  });
  ...
  describe("receive function", async function () {
    ...
    it("should be called and revert if called from low-level transaction", async function () {
      let contractAddress = await RFergNFTContract.getAddress();
  ...
  describe("mintTo function", async function () {
    ...
    it("should revert if called with no ether", async function () {
      const [owner, artist, buyer] = await ethers.getSigners();
      expect(
        RFergNFTContract.connect(buyer).mintTo(args.base_uri, {
    ...
    it("should have a totalSupply = 1 with after first mint", async function () {
      ...
      const mint1 = await RFergNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint1.wait(1);
      const currentValue = await RFergNFTContract.totalSupply();
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should increase the totalSupply by 1 with with each mint", async function () {
      ...
      mint1.wait(1);
      const mint2 = await RFergNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint2.wait(1);
    ...
    it("should emit an event when minting is completed", async function () {
      ...
      expect(
        ...
        .to.emit(RFergNFTContract, "MintingCompleted")
        .withArgs(1, owner);
    });
    ...
    it("should have set the token uri during minting function", async function () {
      ...
      mint1.wait(1);
      const currentValue = await RFergNFTContract.tokenURI(1);
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should emit an event after funds are distributed during mint", async function () {
      ...
      const uriString = "someString";
      expect(
        RFergNFTContract.connect(buyer).mintTo(uriString, {
          value: args.mint_price,
        })
      )
        .to.emit(RFergNFTContract, "FundsDistributed")
